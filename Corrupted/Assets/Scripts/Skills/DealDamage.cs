...
public class DealDamage : MonoBehaviour
{
    ...
    {
        if (other.transform.tag !="Player" && other.transform.tag !="Bullet") //AVOIDS COLLISION WITH OTHER BULLETS OR THE PLAYER
        {
            if (!NoDestroy)
            {
            ...
            {
                Destroy(gameObject, 2f);  //DESTROY THE GAMEOBJECTS AFTER 2f
            }
        }
        if(other.transform.tag == "Enemy") //DETONA CONTRA LOS ENEMIGOS Y HACE DAÑO
        {
